...
temp_dir="$dest_folder/temp_$datetime"
...
backup_differential() {
    echo "Creazione della directory temporanea: $temp_dir"
    mkdir -p "$temp_dir"

    for dir in ${source_dirs//,/ }; do
        echo "Eseguo il backup differenziale della directory $dir in $temp_dir"
        rsync -a --delete "$dir/" "$temp_dir/$(basename "$dir")"
    done

    if [[ "$useTAR" == "true" ]]; then
        echo "Creazione dell'archivio TAR differenziale..."
        tar -cf "$archive_path.tar" -C "$temp_dir" .
        echo "Eliminazione della directory temporanea: $temp_dir"
        rm -rf "$temp_dir"
    else
        echo "Creazione della cartella di backup differenziale: $backup_dir"
        mkdir -p "$backup_dir"
        cp -ar "$temp_dir/"* "$backup_dir/"
        echo "Eliminazione della directory temporanea: $temp_dir"
        rm -rf "$temp_dir"
    fi
}
